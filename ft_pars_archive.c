...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/24 23:11:24 by amovchan          #+#    #+#             */
/*   Updated: 2019/04/24 23:20:30 by amovchan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_pars_archv(t_object *ptr_obj)
{
	...
	while (size)
	{
		...
		magic = *(unsigned int*)((void*)hdr +
				sizeof(struct ar_hdr) + offset);
		if (MH_MAGIC == magic)
			get_archv_arch32(ptr_obj, (void*)hdr +
					sizeof(struct ar_hdr) + offset);
		else if (MH_MAGIC_64 == magic)
			get_archv_arch64(ptr_obj, (void*)hdr +
					sizeof(struct ar_hdr) + offset);
